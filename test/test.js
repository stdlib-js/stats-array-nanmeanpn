...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof nanmeanpn, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( nanmeanpn.length, 1, 'returns expected value' );
	t.end();
});
...
tape( 'if provided an empty array, the function returns `NaN` (accessors)', function test( t ) {
	var v = nanmeanpn( toAccessorArray( [] ) );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
	t.end();
});
...
tape( 'if provided an array containing a single element, the function returns the first element (accessors)', function test( t ) {
	var v = nanmeanpn( toAccessorArray( [ 1.0 ] ) );
	t.strictEqual( v, 1.0, 'returns expected value' );
	t.end();
});
